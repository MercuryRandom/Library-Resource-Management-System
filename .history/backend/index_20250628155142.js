...
app.get('/api/admin/admins', (req, res) => {
  db.query('SELECT admin_id, admin_type, created_at FROM admin ORDER BY admin_id', (err, results) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    res.json(results);
...
app.post('/api/admin/add-admin', (req, res) => {
  const { admin_id, Password, admin_type } = req.body;

  // 数据验证
  if (!admin_id || !Password || !admin_type) {
    return res.json({ success: false, message: '请填写所有必填字段' });
  }

  // 限制admin_type字段
  if (!['普通管理员', '高级管理员'].includes(admin_type)) {
    return res.json({ success: false, message: '管理员类型只能是：普通管理员、高级管理员' });
  }

  db.query(
    'INSERT INTO admin (admin_id, Password, admin_type) VALUES (?, ?, ?)',
    [admin_id, Password, admin_type],
    (err, result) => {
      if (err) {
        if (err.code === 'ER_DUP_ENTRY') {
...
app.put('/api/admin/admins/:admin_id', (req, res) => {
  ...
  const { Password, admin_type } = req.body;

  // 数据验证
  if (!admin_type) {
    return res.json({ success: false, message: '请填写管理员类型' });
  }
  ...
  let query, params;
  if (Password) {
    query = 'UPDATE admin SET Password = ?, admin_type = ? WHERE admin_id = ?';
    params = [Password, admin_type, admin_id];
  } else {
    query = 'UPDATE admin SET admin_type = ? WHERE admin_id = ?';
    params = [admin_type, admin_id];
  }
...
app.get('/api/admin/current', (req, res) => {
  ...
  db.query('SELECT admin_id, admin_type FROM admin WHERE admin_id = ?', [currentAdminId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (results.length === 0) {
