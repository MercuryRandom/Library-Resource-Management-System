...
app.post('/api/admin/add-reader', (req, res) => {
  ...
  // 数据验证
  if (!reader_id || !Password || !name || !contact_info || !reader_type) {
    return res.json({ success: false, message: '请填写所有必填字段' });
  }

  // 限制gender字段长度，只允许"男"、"女"、"其他"
  let validGender = null;
  if (gender) {
    if (['男', '女', '其他'].includes(gender)) {
      validGender = gender;
    } else {
      return res.json({ success: false, message: '性别只能是：男、女、其他' });
    }
  }

  // 限制reader_type字段
  if (!['学生', '教职工'].includes(reader_type)) {
    return res.json({ success: false, message: '读者类型只能是：学生、教职工' });
  }

  db.query(
    'INSERT INTO reader (reader_id, Password, name, gender, contact_info, reader_type) VALUES (?, ?, ?, ?, ?, ?)',
    [reader_id, Password, name, validGender, contact_info, reader_type],
    (err, result) => {
      if (err) {
        if (err.code === 'ER_DUP_ENTRY') {
          return res.json({ success: false, message: '读者ID已存在' });
        }
...
app.post('/api/admin/login', (req, res) => {
  ...
  db.query('SELECT admin_id, username, privilege_level FROM administrator WHERE username=? AND password=?', [username, password], (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    if (results.length > 0) {
...
app.get('/api/recommend/popular-rating', (req, res) => {
  db.query(`SELECT b.book_id, b.title, b.author, 
    ROUND(AVG(br.rating), 1) as avg_rating, COUNT(br.rating) as rating_count
    FROM book b
    ...
    LIMIT 5`, (err, results) => {
    if (err) {
      console.error('热门评分榜查询错误:', err);
      return res.status(500).json({ error: err.message });
    }
    console.log('热门评分榜结果:', results);
    res.json(results);
  });
});
// 3. 热门收藏（预约次数最多，前5名）
...
// 管理员借还管理API
// 1. 获取所有未归还的图书列表
app.get('/api/admin/unreturned-books', (req, res) => {
  db.query(`
    SELECT br.borrow_id, br.reader_id, br.book_id, br.borrow_date, br.due_date,
           r.name as reader_name, r.reader_type,
           b.title, b.author
    FROM borrow_return br
...
// 2. 管理员直接归还图书
app.post('/api/admin/return-book', (req, res) => {
  const { borrow_id } = req.body;
  if (!borrow_id) {
    return res.json({ success: false, message: '缺少借书ID' });
  }

  const return_date = new Date().toISOString().slice(0, 10);

  // 更新借书记录
  db.query('UPDATE borrow_return SET return_date=? WHERE borrow_id=?', [return_date, borrow_id], (err, result) => {
    if (err) return res.status(500).json({ error: err.message });
    if (result.affectedRows === 0) return res.json({ success: false, message: '未找到借书记录' });

    // 更新图书库存
    db.query('UPDATE book SET available_copies = available_copies + 1 WHERE book_id = (SELECT book_id FROM borrow_return WHERE borrow_id=?)', [borrow_id], (err2) => {
      if (err2) return res.status(500).json({ error: err2.message });
...
app.get('/api/admin/admins', (req, res) => {
  db.query('SELECT admin_id, username, contact_info, privilege_level FROM administrator ORDER BY admin_id', (err, results) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    res.json(results);
...
app.post('/api/admin/add-admin', (req, res) => {
  const { admin_id, username, password, contact_info, privilege_level } = req.body;

  // 数据验证
  if (!admin_id || !username || !password || !contact_info || !privilege_level) {
    return res.json({ success: false, message: '请填写所有必填字段' });
  }

  // 限制privilege_level字段
  if (!['普通管理员', '高级管理员'].includes(privilege_level)) {
    return res.json({ success: false, message: '管理员类型只能是：普通管理员、高级管理员' });
  }

  db.query(
    'INSERT INTO administrator (admin_id, username, password, contact_info, privilege_level) VALUES (?, ?, ?, ?, ?)',
    [admin_id, username, password, contact_info, privilege_level],
    (err, result) => {
      if (err) {
        if (err.code === 'ER_DUP_ENTRY') {
          return res.json({ success: false, message: '管理员ID已存在' });
        }
        return res.status(500).json({ error: err.message });
      }
      res.json({ success: true, message: '添加成功' });
...
// 3. 修改管理员
app.put('/api/admin/admins/:admin_id', (req, res) => {
  const { admin_id } = req.params;
  const { username, password, contact_info, privilege_level } = req.body;

  // 数据验证
  if (!username || !contact_info || !privilege_level) {
  ...
  let query, params;
  if (password) {
    query = 'UPDATE administrator SET username = ?, password = ?, contact_info = ?, privilege_level = ? WHERE admin_id = ?';
    params = [username, password, contact_info, privilege_level, admin_id];
  } else {
    query = 'UPDATE administrator SET username = ?, contact_info = ?, privilege_level = ? WHERE admin_id = ?';
    params = [username, contact_info, privilege_level, admin_id];
  }
...
// 4. 删除管理员
app.delete('/api/admin/admins/:admin_id', (req, res) => {
  const { admin_id } = req.params;

  db.query('DELETE FROM administrator WHERE admin_id = ?', [admin_id], (err, result) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (result.affectedRows === 0) {
...
app.get('/api/admin/current', (req, res) => {
  // 这里需要根据你的session管理方式获取当前管理员ID
  // 暂时返回一个示例，你需要根据实际情况修改
  const currentAdminId = req.session?.admin_id || req.headers['admin-id'];

  if (!currentAdminId) {
    return res.status(401).json({ error: '未登录' });
  }

  db.query('SELECT admin_id, username, privilege_level FROM administrator WHERE admin_id = ?', [currentAdminId], (err, results) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    if (results.length === 0) {
