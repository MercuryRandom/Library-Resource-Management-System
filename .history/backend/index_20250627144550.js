...
app.post('/api/reserve', (req, res) => {
  ...
  db.query('SELECT reservation_id FROM reservation ORDER BY reservation_id DESC LIMIT 1', (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    let newId = 'Y001';
    if (results.length > 0) {
    ...
    db.query(
      ...
      (err2, result) => {
        if (err2) return res.status(500).json({ error: err2.message });
        res.json({ success: true, message: '预约成功', reservation_id: newId });
      }
    );
  });
});
