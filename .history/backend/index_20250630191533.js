...
app.get('/api/recommend/popular-borrow', (req, res) => {
  db.query(
    'SELECT book_id, title, author, COUNT(*) as borrow_count FROM borrow_return JOIN book USING(book_id) GROUP BY book_id, title, author ORDER BY borrow_count DESC LIMIT 5',
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
    }
...
// 首页推荐：热门评分榜（如无评分表，返回空数组或模拟数据）
app.get('/api/recommend/popular-rating', (req, res) => {
  // 假设没有评分表，返回空数组或模拟数据
  res.json([
    { book_id: 'B001', title: '高分图书A', author: '作者A', avg_rating: 4.9 },
    { book_id: 'B002', title: '高分图书B', author: '作者B', avg_rating: 4.8 },
    { book_id: 'B003', title: '高分图书C', author: '作者C', avg_rating: 4.7 }
  ]);
});
...
app.get('/api/recommend/popular-collect', (req, res) => {
  db.query(
    'SELECT book.book_id, book.title, book.author, COUNT(*) as reservation_count FROM reservation JOIN book ON reservation.book_id = book.book_id GROUP BY book.book_id, book.title, book.author ORDER BY reservation_count DESC LIMIT 5',
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
    }
...
// 首页推荐：新书通报（按出版日期分组，取最新的若干本）
app.get('/api/recommend/new-books', (req, res) => {
  db.query(
    'SELECT * FROM book ORDER BY publication_date DESC LIMIT 20',
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      // 按分类分组
      const grouped = {};
      results.forEach(book => {
        const cat = book.classification_num || '其他';
