...
app.post('/api/register', (req, res) => {
  const { contact_info, password, name, gender, reader_type } = req.body;
  if (!contact_info || !password || !name || !reader_type) {
    return res.json({ success: false, message: '信息不完整' });
  }

  // 1. 检查手机号是否已存在
  db.query('SELECT * FROM reader WHERE contact_info = ?', [contact_info], (err, results) => {
    if (err) {
    ...
    // 2. 生成新的 reader_id
    db.query("SELECT reader_id FROM reader WHERE reader_id LIKE 'R%' ORDER BY CAST(SUBSTRING(reader_id, 2) AS UNSIGNED) DESC LIMIT 1", (err, results) => {
      if (err) {
        return res.status(500).json({ success: false, error: err.message });
      }
      ...
      if (results.length > 0) {
        const lastId = results[0].reader_id;
        const lastNum = parseInt(lastId.substring(1), 10);
        const newNum = lastNum + 1;
        newReaderId = 'R' + String(newNum).padStart(3, '0');
      }
