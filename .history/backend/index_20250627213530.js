...
app.get('/api/history/:reader_id', (req, res) => {
  ...
  db.query(
    'SELECT br.*, b.title FROM borrow_return br LEFT JOIN book b ON br.book_id = b.book_id WHERE br.reader_id=? ORDER BY br.borrow_date DESC',
    [reader_id],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
...
app.post('/api/rate', (req, res) => {
  ...
  db.query('SELECT * FROM borrow_return WHERE reader_id=? AND book_id=? AND return_date IS NOT NULL', [reader_id, book_id], (err, rows) => {
    ...
    // 检查是否已评分（用borrow_return表的一个字段或单独表，这里用book表+前端限制）
    db.query('SELECT rated_books FROM reader WHERE reader_id=?', [reader_id], (err2, rRows) => {
      let ratedBooks = [];
      if (!err2 && rRows.length > 0 && rRows[0].rated_books) {
        try { ratedBooks = JSON.parse(rRows[0].rated_books); } catch { ratedBooks = []; }
      }
      if (ratedBooks.includes(book_id)) {
        return res.json({ success: false, message: '您已评价过该书' });
      }
      // 取当前平均分和人数
      db.query('SELECT rating, rating_count FROM book WHERE book_id=?', [book_id], (err3, bRows) => {
        if (err3) return res.status(500).json({ error: err3.message });
        let oldAvg = bRows[0].rating || 0;
        let oldCount = bRows[0].rating_count || 0;
        let newCount = oldCount + 1;
        let newAvg = ((oldAvg * oldCount) + Number(rating)) / newCount;
        // 更新book表
        db.query('UPDATE book SET rating=?, rating_count=? WHERE book_id=?', [newAvg, newCount, book_id], (err4) => {
          if (err4) return res.status(500).json({ error: err4.message });
          // 更新用户已评分书籍
          ratedBooks.push(book_id);
          db.query('UPDATE reader SET rated_books=? WHERE reader_id=?', [JSON.stringify(ratedBooks), reader_id]);
          res.json({ success: true, message: '评分成功', rating: newAvg });
        });
      });
    });
  });
...
app.get('/api/recommend/popular-rating', (req, res) => {
  db.query(`SELECT book_id, title, author, rating FROM book WHERE rating > 0 ORDER BY rating DESC LIMIT 10`, (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(results);
  });
});
...
app.get('/api/recommend/popular-collect', (req, res) => {
  db.query(`SELECT book_id, title, author, reservation_count FROM book ORDER BY reservation_count DESC LIMIT 10`, (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(results);
  });
});
