...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:4001/api/recommend/new-books').then(r => r.json()).then(setNewBooks);
  }, []);
    ...
    <ul style={{ padding: 0, margin: 0, listStyle: 'none', fontSize: 15 }}>
      {books.map(book => (
        <li key={book.book_id || book.id} style={{ marginBottom: 8 }}>
          ...
          {type === 'collect' && <span style={{ color: '#2d6cdf', marginLeft: 8 }}>收藏 {book.collect_count || 0} 次</span>}
        </li>
      ))}
    </ul>
  </div>
