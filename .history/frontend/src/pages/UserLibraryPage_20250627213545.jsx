...
const UserLibraryPage = () => {
  ...
  const RatingCell = ({ reader_id, book_id, onRated, disabled }) => {
    ...
    const handleRate = async () => {
      ...
      try {
        ...
        if (data.success) {
          setMsg('评分成功');
          setTempRatings(r => ({ ...r, [book_id]: rating }));
          setShow(false);
          onRated && onRated();
        } else {
    ...
    return show ? (
      ...
      <button onClick={() => setShow(true)} disabled={disabled} style={{ background: disabled ? '#ccc' : '#2d6cdf', color: '#fff', border: 'none', borderRadius: 6, padding: '2px 10px', fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer' }}>评分</button>
    );
  };
                    ...
                    <td>
                      {item.return_date ? (
                        item.rated || item.rating ? (
                          ...
                          <RatingCell reader_id={reader_id} book_id={item.book_id} onRated={() => setHistory(h => h.map(hh => hh.book_id === item.book_id ? { ...hh, rated: true, rating: tempRatings[item.book_id] || 5 } : hh))} disabled={!!item.rated} />
                        )
                      ) : '--'}
                    </td>
                  </tr>
