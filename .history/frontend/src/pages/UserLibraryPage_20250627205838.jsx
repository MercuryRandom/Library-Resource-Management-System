  ...
  const RatingCell = ({ reader_id, book_id, onRated, disabled }) => {
    ...
    return show ? (
      <span>
        ...
        <button onClick={handleRate} disabled={loading}>提交</button>
        <button onClick={() => setShow(false)} style={{ marginLeft: 4 }}>取消</button>
        {msg && <span style={{ color: msg.includes('成功') ? 'green' : 'red', marginLeft: 6 }}>{msg}</span>}
      </span>
    ) : (
      <button onClick={() => setShow(true)} disabled={disabled}>评分</button>
    );
  };
