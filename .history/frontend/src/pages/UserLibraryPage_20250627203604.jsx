  ...
  const RatingCell = ({ reader_id, book_id, onRated }) => {
    ...
    return show ? (
      <span>
        ...
        <button onClick={handleRate} disabled={loading} style={{ marginLeft: 6 }}>提交</button>
      ...
      <button onClick={() => setShow(true)}>评分</button>
    );
  };
                    ...
                    <td>
                      {item.return_date ? (
                        item.rating ? (
                          ...
                          <RatingCell reader_id={reader_id} book_id={item.book_id} onRated={() => setHistory(h => h.map(hh => hh.book_id === item.book_id ? { ...hh, rating: tempRatings[item.book_id] || 5 } : hh))} />
                        )
                      ) : '--'}
                    </td>
                  </tr>
