...
const HomePage = () => {
  ...
  useEffect(() => {
    // 获取热门借阅榜
    fetch('http://localhost:4001/api/recommend/popular-borrow')
      .then(r => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      })
      .then(data => {
        console.log('热门借阅榜数据:', data);
        setPopularBorrow(data);
      })
      .catch(err => {
        console.error('获取热门借阅榜失败:', err);
        setPopularBorrow([]);
      });

    // 获取热门评分榜
    fetch('http://localhost:4001/api/recommend/popular-rating')
      .then(r => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      })
      .then(data => {
        console.log('热门评分榜数据:', data);
        setPopularRating(data);
      })
      .catch(err => {
        console.error('获取热门评分榜失败:', err);
        setPopularRating([]);
      });

    // 获取热门收藏榜
    fetch('http://localhost:4001/api/recommend/popular-collect')
      .then(r => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      })
      .then(data => {
        console.log('热门收藏榜数据:', data);
        setPopularCollect(data);
      })
      .catch(err => {
        console.error('获取热门收藏榜失败:', err);
        setPopularCollect([]);
      });

    // 获取新书通报
    fetch('http://localhost:4001/api/recommend/new-books')
      .then(r => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json();
      })
      .then(data => {
        console.log('新书通报数据:', data);
        setNewBooks(data);
        const cats = Object.keys(data);
        if (cats.length > 0) setCategory(cats[0]);
      })
      .catch(err => {
        console.error('获取新书通报失败:', err);
        setNewBooks({});
      });
  }, []);
        ...
        <div>
          {(newBooks[category] || []).length === 0 ? <div style={{ color: '#888' }}>暂无新书。</div> : (
            <ul style={{ padding: 0, margin: 0, listStyle: 'none', display: 'flex', gap: 18 }}>
              {(newBooks[category] || []).slice(0, 4).map(book => (
                <li key={book.book_id} style={{ background: 'linear-gradient(135deg,#e6f0ff 0%,#f5f7fb 100%)', borderRadius: 12, boxShadow: '0 2px 8px #0001', padding: '18px 20px', minWidth: 160, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', transition: 'box-shadow 0.2s', border: '1.5px solid #e0e7f7' }}>
                  <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 6, color: '#2d6cdf', wordBreak: 'break-all' }}>{book.title}</div>
                  <div style={{ color: '#666', fontSize: 15, marginBottom: 2 }}>{book.author}</div>
                </li>
              ))}
            </ul>
          )}
    ...
    <ul style={{ padding: 0, margin: 0, listStyle: 'none', fontSize: 15 }}>
      {books.map(book => (
        <li key={book.book_id || book.id} style={{ marginBottom: 8 }}>
          ...
          {type === 'rating' && <span style={{ color: '#f7b500', marginLeft: 8 }}>评分 {parseFloat(book.avg_rating || 0).toFixed(1)} 分</span>}
